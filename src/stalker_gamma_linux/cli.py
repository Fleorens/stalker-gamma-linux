...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Couches de performance (T20) : toutes désactivées par défaut. Elles
    # changent le rendu ou les performances — un joueur qui ouvre une issue
    # doit pouvoir dire « rien d'activé » sans avoir à le vérifier.
    play_parser.add_argument(
        "--mangohud",
        action="store_true",
        help=_("Displays the MangoHud overlay (Vulkan layer, off by default)"),
    )
    play_parser.add_argument(
        "--mangohud-preset",
    ...
    play_parser.add_argument(
        ...
        action="store_true",
        help=_(
            "Enables vkBasalt with our « ReShade-like » preset "
            "(sharpening + colour grading), the Linux answer to the removed ReShade"
        ),
    )
...
def _dispatch(args: argparse.Namespace) -> int:
    ...
    if args.command == "play":
        return run_play(
            ...
            performance=_performance_settings(args),
        )
